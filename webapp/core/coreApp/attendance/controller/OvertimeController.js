Ext.define("core.attendance.controller.OvertimeController", {
    ...
    init: function () {
        ...
        var editRecord= function (record) {
            var win = Ext.widget("overtimeWindow");
        ...
        var applyStatus = function (_btn, status) {
            ...
            Ext.MessageBox.confirm(
                "请确认"
                , "确定"+(status===1?"通过":"拒绝")+"选中的加班申请吗？", function (btn) {
                    if (btn == 'yes') {
                        // 根据id删除多条记录
                        var data = [];
                        ...
                        Ext.Ajax.request({
                            waitMsg: '正在进行处理,请稍后...',
                            url: _hostUrl + "/v1/overtime/status/"+ status,
                            params: {
                                overIds: data.join(',')
                            },
                            method: "PUT",
        ...
        this.control({
             ...
             */
            "overtimeGrid button[ref=updateOvertime]": {
                click: function (_btn) {
                    var grid = _btn.up("overtimeGrid");
                    var records = grid.getSelectionModel().getSelection();
             ...
             * 通过申请
             */
            "overtimeGrid button[ref=accept]": {
                click: function (_btn) {
                    applyStatus(_btn,1);
                }
            },
             ...
             * 拒绝申请
             */
            "overtimeGrid button[ref=refuse]": {
                click: function (_btn) {
                    applyStatus(_btn,2);
                }
            },
             ...
             * 查找功能
             */
            "overtimeGrid button[ref=search]": {
                click: function (_btn) {
                    ...
                    //临时设置请求地址
                    store.getProxy().url = _hostUrl + "/v1/overtimes/query";
             ...
             * 添加用户
             */
            "overtimeGrid button[ref=addOvertime]": {
                click: function (btn) {
                    var win = Ext.widget("overtimeWindow");
                    win.setTitle("添加加班申请");
                    var form = win.down("form");
             ...
             * 删除栏目
             */
            "overtimeGrid button[ref=removeOvertime]": {
                click: function (btn) {
                    var grid = btn.up("overtimeGrid");
                    var flag = true;
                    ...
                    commonDelete(grid, "/v1/overtimes");
                }
            },
            /**
             * 刷新按钮
             */
            "overtimeGrid button[ref=refresh]": {
                click: function (btn) {
             ...
             * 添加菜单form的保存按钮
             */
            "overtimeWindow button[ref=save]": {
                click: function (btn) {
                    //1获得form
                    ...
                    var endTime = getTimeField(_form, 'endDate', 'endTimeStr');
                    //2.把数据保存到数据库中去
                    _form.submit({
                        ...
                        failure: function (form, action) {
                            ...
                            if (resObj.code == 201) {
                                Ext.getCmp("overtimeWindow").close();
                                var _grid = Ext.widget("overtimeGrid");
            ...
            //重置事件
            "overtimeWindow button[ref=reset]": {
                click: function (btn) {
             ...
             * 添加用户form的返回按钮
             */
            "overtimeWindow button[ref=return]": {
                click: function (btn) {
                    Ext.getCmp("overtimeWindow").close();
                }
            },
            "overtimeGrid combobox[ref=overtimePageSize]": {
                change: function (_this, newValue, oldValue, eOpts) {
                    var store = _this.up("overtimeGrid").getStore();
    ...
    views: [
        "core.attendance.view.OvertimeLayout",
        "core.attendance.view.OvertimeGrid",
        "core.attendance.view.OvertimeWindow"
    ],
    stores: ["core.attendance.store.OvertimeStore"],
    models: ["core.attendance.model.OvertimeModel"]
});
//计算加班时间
setTimeCount = function (obj) {
    var _form = obj.ownerCt.ownerCt;
    var timeCount = _form.getForm().findField('timeCount');
    var startTime = getTimeField(_form, 'startDate', 'startTimeStr');
    var endTime = getTimeField(_form, 'endDate', 'endTimeStr');
    //结束时间戳
    ...
    if (count < 0) {
        ...
        timeCount.setValue(count / (1000 * 60 * 60));
    }
};
